...
class cls_template
{
    ...
    function fetch($filename, $cache_id = '')
    {
        ...
        $out=str_replace('"/skinp','"'.CDN_URL."/skinp",$out);
		$out=str_replace("'/skinp","'".CDN_URL."/skinp",$out);
		$out=str_replace('"/scripts','"'.CDN_URL."/scripts",$out);
		$out=str_replace("'/scripts","'".CDN_URL."/scripts",$out);
        ...
        else
        {
            ...
            switch ($tag_sel)
            {
	            ...
	            case 'info':
			        ...
			        return $this->_compile_info_start(substr($tag, 5));
...
function _compile_info_start($tag_args)
    {
    $attrs = $this->get_para($tag_args, 0);
	$from = $attrs['from'];
	$item =$attrs['item'];
	$output = '<?php ';
    $output .= "\$_from=json_encode($from);  \$_from2=json_decode(\$_from);   echo \$_from2->$item;" ;
    return $output . '?>';
	}
    /**
     * 处理foreach标签
...
$source = str_replace('</body>', "\n<script type=\"text/javascript\" src=\"/js/extral.js\"></script>\n</body>\n", $source);
