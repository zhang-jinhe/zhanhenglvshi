 ...
 * @return  void
 */
function set_admin_session($user_id, $username, $action_list, $city,$role_id,$articlecatroles,$adcatroles,$last_time)
{
    $_SESSION['admin_id']    = $user_id;
    $_SESSION['admin_name']  = $username;
    $_SESSION['action_list'] = $action_list;
    $_SESSION['city'] = isset($city)?$city:0;
	$_SESSION['role_id'] = isset($role_id)?$role_id:0;
	$_SESSION['articlecatroles'] = !empty($articlecatroles)?$articlecatroles:'';
	$_SESSION['goodscatroles'] = !empty($goodscatroles)?$goodscatroles:'';
	$_SESSION['adcatroles'] = !empty($adcatroles)?$adcatroles:'';
...
function create_html_editor($input_name, $input_value = '')
{
    ...
    initialFrameHeight:200  //初始化编辑器高度,默认180
    });
    </script>';
...
function create_html_bdeditor($input_name, $input_value = '')
{
    global $smarty;
    /* mod by wengwenjin 百度编辑器 begin */
    $HTML='
  <script type="text/javascript" charset="utf-8" src="../../includes/ueditor/ueditor.config.js"></script>
    ...
    $smarty->assign('bdeditor', $HTML);
    /* mod by wengwenjin 百度编辑器 end */
}

function create_html_bdeditor2($input_name, $input_value = '')
{
    global $smarty;
    /* mod by wengwenjin 百度编辑器 begin */
    $HTML='
...
function get_articlecat_openlist() {
	$articlecat = article_cat_list(0, 0, false);
	foreach($articlecat as $key => $cat) {
		$articlecat_cat_id[] = $cat['cat_id'];
	} 

	if ($GLOBALS['_OPE']['article_cat']) {
		$hiden_article_cat = (int)$GLOBALS['_OPE']['article_cat'];
		$hiden_item_list = array_unique(array_keys(article_cat_list($hiden_article_cat, 0, false)));
		$articlecat_cat_id = array_diff($articlecat_cat_id, $hiden_item_list);
	} 

	foreach($articlecat as $key => $cat) {
		if (in_array($key, $articlecat_cat_id)) {
			$arr[$key] = $cat;
