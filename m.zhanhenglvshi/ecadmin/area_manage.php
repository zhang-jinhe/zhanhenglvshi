...
elseif ($_REQUEST['act'] == 'edit_area_desc')
{
    check_authz_json('area_manage');

    $id = intval($_POST['id']);
    $region_desc = json_str_iconv(trim($_POST['val']));
    ...
    if ($exc->edit("region_desc = '$region_desc'", $id))
    {
        admin_log($region_name, 'edit', 'area');
        make_json_result(stripslashes($region_desc));
    }
    else
    {
        make_json_error($db->error());
